...
// A specific, independently verified schedule for exactly 8 players,
// 2 courts, 7 rounds — hardcoded rather than generated, because this
// exact combination has a real, provable mathematical obstruction: no
// schedule can be simultaneously perfect on partnerships, opponents,
// AND court balance for n=8. Verified by an exhaustive search across
// hundreds of structurally distinct candidate schedules, cross-checked
// against the published combinatorics literature on "balanced whist
// tournaments" (a known, separate, and harder property than an
// ordinary whist tournament design). This schedule keeps the two
// properties that matter most for fairness of play — every player
// partners every other player exactly once, and opposes every other
// player exactly twice — while using the best court balance achievable
// under that constraint: nobody is stuck on one court the whole
// tournament (worst case is 1-of-7 vs 6-of-7), most players land on
// the ideal 3-4 or 4-3 split. Kept identical to rally.html's copy
// deliberately, same as every other part of this algorithm.
const WHIST_8_SCHEDULE = [
  [[2, 3], [4, 6], [5, 1], [7, 0]],
  [[1, 2], [3, 5], [4, 7], [6, 0]],
  [[7, 1], [2, 4], [3, 6], [5, 0]],
  [[6, 7], [1, 3], [2, 5], [4, 0]],
...
function whist8Schedule(players) {
  return WHIST_8_SCHEDULE.map((round) => {
    const [court1TeamA, court1TeamB, court2TeamA, court2TeamB] = round;
    const toIds = (pair) => pair.map((idx) => players[idx].id);
    return {
      courts: [
        { court: 1, teamA: toIds(court1TeamA), teamB: toIds(court1TeamB), scoreA: null, scoreB: null },
        { court: 2, teamA: toIds(court2TeamA), teamB: toIds(court2TeamB), scoreA: null, scoreB: null },
      ],
      sittingOut: [],
    };
  });
...
function generatePerfectAmericanoSchedule(players, numCourts, numRounds, balanceAttempts = 200) {
  ...
  if (n === 8 && numCourts === 2 && numRounds === 7) {
    return whist8Schedule(players);
  }
